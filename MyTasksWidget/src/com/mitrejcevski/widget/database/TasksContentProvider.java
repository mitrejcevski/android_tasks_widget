...
public class TasksContentProvider extends ContentProvider {
	...
	static {
		sUriMatcher
				.addURI(ContentData.AUTHORITY, GroupsTable.PATH, CODE_GROUPS);
		sUriMatcher.addURI(ContentData.AUTHORITY, GroupsTable.PATH + "/#",
				CODE_GROUP_ID);
		sUriMatcher.addURI(ContentData.AUTHORITY, TasksTable.PATH, CODE_TASKS);
		sUriMatcher.addURI(ContentData.AUTHORITY, TasksTable.PATH + "/#",
				CODE_TASK_ID);
	}
	...
	public int delete(Uri uri, String selection, String[] selectionArgs) {
		...
		case CODE_GROUPS:
			count = database.delete(GroupsTable.TABLE_NAME, selection,
					selectionArgs);
			break;
		case CODE_GROUP_ID:
			count = database.delete(
					...
					ContentData._ID + " = ?",
					new String[] { uri.getPathSegments().get(
							ContentData.ID_PATH_POSITION) });
			break;
		case CODE_TASKS:
			count = database.delete(TasksTable.TABLE_NAME, selection,
					selectionArgs);
			break;
		case CODE_TASK_ID:
			count = database.delete(
